...
2. Comparison view in readable layout (terminal-style ASCII table by default).
...
1. Terminal-style ASCII table in fenced `text` block.
2. Split into two smaller ASCII tables.
3. Compact card/list view by variant.

Markdown tables are forbidden for user-facing output.

Trigger fallback when:
- user says "поехало", "разъехалось", "непонятно", "перерисуй";
- layout is too wide for current client;
- comparison is too dense for one table.
...
## Save Flow
When user says "save", run save flow directly through CLI commands and confirm what was saved.
